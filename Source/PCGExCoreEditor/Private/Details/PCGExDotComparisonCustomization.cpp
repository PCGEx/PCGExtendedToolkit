	...
	IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	[
		...
		[
			...
			{
				bool Value = false;
				if (UnsignedHandle.IsValid())
				{
					UnsignedHandle->GetValue(Value);
				}
				return Value;
			})
...
double FPCGExDotComparisonCustomization::GetComparisonThreshold() const
{
	// Read domain
	uint8 DomainValue = 0;
	if (DomainHandle.IsValid())
	{
		DomainHandle->GetValue(DomainValue);
	}
	...
	{
		double Degrees = 90.0;
		if (DegreesConstantHandle.IsValid())
		{
	...
	else
	{
		...
		{
			double Dot = 0.5;
			if (DotConstantHandle.IsValid())
			{
				DotConstantHandle->GetValue(Dot);
			}
			RawDot = Dot;
		}
		else
...
bool FPCGExDotComparisonCustomization::IsAttributeMode() const
{
	...
	{
		return false;
	} // Static variant is always constant
