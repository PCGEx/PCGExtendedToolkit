...
FVector FPCGExNoiseSpots::GetSpotCenter(const int32 CellX, const int32 CellY, const int32 CellZ, const uint32 CellHash) const
{
	const FVector J = Hash32ToVector01(CellHash);

	return FVector(
		CellX + 0.5 + (J.X - 0.5) * Jitter * 2.0,
		CellY + 0.5 + (J.Y - 0.5) * Jitter * 2.0,
		CellZ + 0.5 + (J.Z - 0.5) * Jitter * 2.0
		);
}

double FPCGExNoiseSpots::GetSpotRadius(const uint32 CellHash) const
{
	...
	const double Variation = (Hash32ToDouble01(Hash32Remix(CellHash)) - 0.5) * 2.0 * RadiusVariation;
	return FMath::Clamp(SpotRadius + Variation, 0.05, 0.9);
}

double FPCGExNoiseSpots::GetSpotValue(const uint32 CellHash) const
{
	...
	// Double remix keeps the value channel decorrelated from the radius channel
	return 1.0 - Hash32ToDouble01(Hash32Remix(Hash32Remix(CellHash))) * ValueVariation;
}
	...
	for (int32 DZ = -1; DZ <= 1; ++DZ)
	{
		for (int32 DY = -1; DY <= 1; ++DY)
		{
			for (int32 DX = -1; DX <= 1; ++DX)
			{
				...
				const uint32 CellHash = Hash32(NX + Seed, NY, NZ);
				const FVector Center = GetSpotCenter(NX, NY, NZ, CellHash);
				const double Radius = GetSpotRadius(CellHash);
