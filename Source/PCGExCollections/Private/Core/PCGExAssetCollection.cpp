...
void UPCGExAssetCollection::EDITOR_FinalizeStagingRebuild(bool bHasChanges)
{
	...
	{
		// Nothing changed: pipelines still get their post hook; the native post work would dirty the
		// package for nothing. A post-hook mutation promotes the session (native post runs last).
...
bool UPCGExAssetCollection::EDITOR_RestageEntryIfChanged(FPCGExAssetCollectionEntry* InEntry, int32 EntryIndex, bool bRecursive)
{
	...
	// Refresh even when staging is identical, or the entry re-reports stale forever -- but never
	// clobber a good baseline with 0, or it oscillates and dirties on every rebuild.
