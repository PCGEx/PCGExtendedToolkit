...
TArray<FPCGPinProperties> UPCGExAssetStagingSettings::OutputPinProperties() const
{
	...
	// Micro-cache redistribution never re-picks entries, so it can't surface sockets.
	if (bDoOutputSockets && !IsMicroRedistribution())
	{
...
bool FPCGExAssetStagingElement::Boot(FPCGExContext* InContext) const
{
	...
	// In micro-cache redistribution, fitting, weight, entry-type and socket outputs don't apply.
	const bool bMicroRedistribute = Settings->IsMicroRedistribution();
	...
	if (Settings->bWriteEntryType && !bMicroRedistribute)
	{
	...
	if (Settings->SelectorMode != EPCGExSelectorMode::Legacy)
	{
		...
		if (PCGExFactories::GetInputFactories<UPCGExSelectorFactoryData>(Context, PCGExCollections::Labels::SourceSelectorLabel, Factories, {PCGExFactories::EType::Selector}, !bMicroRedistribute))
		{
			if (Factories.Num() != 1)
			{
	...
	if (Settings->SourceMode == EPCGExDistributeSourceMode::CollectionMap)
	{
		...
		// map pin, then prepare for redistribution according to RedistributionMode.
		...
		if (Settings->RedistributionMode == EPCGExRedistributionMode::MicroCache)
		{
			// Entry micro caches are built by LoadCache, which UnpackPin already triggered for
			// every mapped collection (via GetValidEntryNum).

			// Inline-toggled settings can't be hidden per-mode (single-bool EditCondition limit),
			// so warn when enabled ones are ignored.
			if (Settings->bWriteEntryType)
			{
				PCGE_LOG(Warning, GraphAndLog, FTEXT("'Write Entry Type' is ignored in Redistribute Micro Cache mode."));
			}
			if (Settings->bDoOutputSockets)
			{
				PCGE_LOG(Warning, GraphAndLog, FTEXT("'Output Sockets' is ignored in Redistribute Micro Cache mode."));
			}
			if (Settings->bWriteTranslation)
			{
				PCGE_LOG(Warning, GraphAndLog, FTEXT("'Write Translation' is ignored in Redistribute Micro Cache mode (no fitting is applied)."));
			}

			// The map contains the full flat host set and RegisterTrackingKeys walks whole
			// subtrees -- register each subtree once, from its first-seen root.
			TSet<uint32> CoveredGUIDs;
			bool bAnyMicroCache = false;
			for (const TPair<uint32, UPCGExAssetCollection*>& Pair : Context->CollectionPickUnpacker->GetCollections())
			{
				...
				if (!CoveredGUIDs.Contains(Host->GetCollectionGUID()))
				{
					Host->EDITOR_RegisterTrackingKeys(Context);
					for (const TObjectPtr<UPCGExAssetCollection>& Flat : Host->GetFlatHosts())
					{
						if (Flat)
						{
							CoveredGUIDs.Add(Flat->GetCollectionGUID());
						}
					}
				}
			...
			if (!bAnyMicroCache && !Settings->bQuietEmptyCollectionError)
			{
				PCGE_LOG(Warning, GraphAndLog, FTEXT("The provided Collection Map contains no refreshable (mesh) micro cache entries -- all points will be passed through untouched."));
			}
		}
		else
		{
			// Redistribute Collections: index every sub-collection entry so points staged on one
	...
	if (Settings->bDoOutputSockets && !bMicroRedistribute)
	{
...
FString UPCGExAssetStagingSettings::GetDisplayName() const
{
	...
	if (SourceMode == EPCGExDistributeSourceMode::CollectionMap)
	{
		DisplayName = RedistributionMode == EPCGExRedistributionMode::MicroCache ? TEXT("Micro Cache") : TEXT("Collection Map");
	}
...
namespace PCGExAssetStaging
{
	bool FProcessor::Process(const TSharedPtr<PCGExMT::FTaskManager>& InTaskManager)
	{
		...
		const bool bMapMode = (Settings->SourceMode == EPCGExDistributeSourceMode::CollectionMap);
		const bool bMicroRedistribute = bMapMode && Settings->RedistributionMode == EPCGExRedistributionMode::MicroCache;

		bApplyFitting = Settings->bApplyFitting && !bMicroRedistribute;
		...
		if (bMicroRedistribute)
		{
			...
			// Resolve each unique staged hash once into MicroTargets; the parallel loop then routes
			// through MicroTargetIndex with no map lookups or virtual dispatch. The entry type
			// filter gates which entries get refreshed.
			...
			TMap<uint64, int32> TargetByHash;
			for (int32 i = 0; i < NumPoints; i++)
			{
				const uint64 Hash = static_cast<uint64>(StagedHashReader->Read(i));
				if (Hash == 0 || Hash == static_cast<uint64>(-1))
				{
					continue; // 0 = never staged, -1 = unstaged sentinel (see InvalidPoint)
				}

				int32 TargetIdx;
				if (const int32* Known = TargetByHash.Find(Hash))
				{
					TargetIdx = *Known;
				}
				else
				{
					...
					if (Result.IsValid() && (!bFilterEntryType || EntryTypeFilter.Matches(Result.Entry->GetTypeId())))
					{
						if (const PCGExMeshCollection::FMicroCache* Refreshable = PCGExCollections::GetRefreshableMicroCache(Result.Entry))
						{
							...
							TargetIdx = MicroTargets.Num() - 1;
						}
					}

					TargetByHash.Add(Hash, TargetIdx);
				}
				...
				// Lazily allocated on the first refreshable pick -- pass-through inputs skip the
				// full-size routing array entirely.
				if (MicroTargetIndex.IsEmpty())
				{
					MicroTargetIndex.Init(-1, NumPoints);
				}
				MicroTargetIndex[i] = TargetIdx;
			}
	...
	{
		// Dispatch before the trace scope so micro-mode scopes emit a single, correctly-labeled event.
		if (MicroRedistributeHelper)
		{
			ProcessPointsMicroRedistribute(Scope);
			return;
		}
	...
	void FProcessor::ProcessPointsMicroRedistribute(const PCGExMT::FScope& Scope)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(PCGEx::AssetStaging::ProcessPointsMicroRedistribute);
		...
		const int32* TargetIndices = MicroTargetIndex.GetData();
		PCGExData::TBuffer<int64>* LocalHashWriter = HashWriter.Get();
		PCGExData::TBuffer<FSoftObjectPath>* LocalPathWriter = PathWriter.Get();
		PCGExCollections::FMicroSelectorHelper* MicroHelper = MicroRedistributeHelper.Get();

		int32 LocalHighestSlotIndex = 0;

		PCGEX_SCOPE_LOOP(Index)
		{
			const int32 TargetIdx = TargetIndices[Index];
			if (TargetIdx == -1 || !PointFilterCache[Index])
			{
				continue;
			}

			const FMicroRefreshTarget& Target = Targets[TargetIdx];
			...
			const int16 SecondaryIndex = static_cast<int16>(NewPick);

			LocalHashWriter->SetValue(Index, static_cast<int64>(PCGExCollections::PickHash::Pack(Target.HostGUID, Target.RawEntryIndex, SecondaryIndex)));
			if (LocalPathWriter)
			{
				LocalPathWriter->SetValue(Index, Target.Entry->Staging.Path);
			}

			if (bLocalPickMaterials)
			{
				MaterialPick[Index] = SecondaryIndex;
				CachedPicks[Index] = Target.Entry;
				LocalHighestSlotIndex = FMath::Max(LocalHighestSlotIndex, Target.HighestSlotIndex);
			}
		}
