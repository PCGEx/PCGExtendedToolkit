...
{
	if (bUseStagedPoints) { PCGEX_PIN_PARAM(PCGExCollections::Labels::SourceCollectionMapLabel, "Collection map generated by staging nodes.", Required) }
	else { PCGEX_PIN_PARAM(PCGExCollections::Labels::SourceCollectionMapLabel, "Collection map generated by staging nodes.", Advanced) }
...
bool FPCGExPathSplineMeshElement::PostBoot(FPCGExContext* InContext) const
{
	...
	// No main collection
	if (Settings->bUseStagedPoints || Settings->CollectionSource == EPCGExCollectionSource::Attribute) { return true; }

	if (!Context->MainCollection) { return false; }
...
bool FPCGExPathSplineMeshElement::AdvanceWork(FPCGExContext* InContext, const UPCGExSettings* InSettings) const
{
	...
	PCGEX_ON_INITIAL_EXECUTION
	{
		...
		{
			if (!StartBatches(InContext->GetOrCreateHandle())) { return true; }
		}
	}
	...
	PCGEX_ON_ASYNC_STATE_READY(PCGExCommon::States::State_WaitingOnAsyncWork)
	{
		...
		for (const TPair<PCGExValueHash, TObjectPtr<UPCGExAssetCollection>>& Pair : Context->CollectionsLoader->AssetsMap)
		{
			...
			if (Pair.Value->GetTypeId() != PCGExAssetCollection::TypeIds::Mesh) { continue; }
		...
		PCGExHelpers::Load(
			...
			[Paths = Context->AssetPaths]() { return Paths->Array(); },
			[&, Ctx = Context->GetOrCreateHandle()](const bool bSuccess, TSharedPtr<FStreamableHandle> StreamableHandle)
			{
				...
				else { SharedContext.Get()->CancelExecution("Could not load any assets"); }
			}
		);
	}
...
namespace PCGExPathSplineMesh
{
	...
	bool FProcessor::Process(const TSharedPtr<PCGExMT::FTaskManager>& InTaskManager)
	{
		...
		if (Settings->bUseStagedPoints)
		{
			...
			if (Settings->bReadTranslation)
			{
				...
				if (!TranslationGetter) { return false; }
			}
		}
		else
		{
	...
	void FProcessor::ProcessPoints(const PCGExMT::FScope& Scope)
	{
		...
		PCGEX_SCOPE_LOOP(Index)
		{
			...
			{
				if (TranslationGetter) { OutTranslation = TranslationGetter->Read(Index); }
			}
	...
	void FProcessor::HandleInvalidPoint(const int32 Index) const
	{
		...
		if (bOutputWeight)
		{
			...
			else { PointDataFacade->GetOut()->GetDensityValueRange(false)[Index] = 0; }
		}
	}
}
