...
void UPCGExPathSplineMeshSettings::InputPinPropertiesBeforeFilters(TArray<FPCGPinProperties>& PinProperties) const
{
	if (bUseStagedPoints)
	{
		PCGEX_PIN_PARAMS(PCGExCollections::Labels::SourceCollectionMapLabel, "Collection map generated by staging nodes.", Required)
	}
	else
	{
		PCGEX_PIN_PARAMS(PCGExCollections::Labels::SourceCollectionMapLabel, "Collection map generated by staging nodes.", Advanced)
	}
...
bool FPCGExPathSplineMeshElement::Boot(FPCGExContext* InContext) const
{
	...
	if (!Settings->bUseStagedPoints)
	{
		...
		else
		{
			...
			Context->SelectorFactory = PCGExCollections::BuildLegacyFactory(Context, Settings->DistributionSettings, Settings->MaterialDistributionSettings);
		}
