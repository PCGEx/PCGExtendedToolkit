...
namespace PCGExData::Helpers
{
	...
	/**
	 * Name overload for values only a desc can type (Struct, Enum, containers...): InValue points to memory of InDesc's
	 * type. Same name rules, slot model and replacement of a same-named attribute of another type; InDesc.Name is ignored.
	 */
	PCGEXCORE_API bool SetDataValue(UPCGData* InData, FName Name, const FPCGMetadataAttributeDesc& InDesc, const void* InValue);
...
extern template bool SetDataValue<_TYPE>(FPCGMetadataAttributeBase* Attribute, const _TYPE Value); \
