 ...
 * placeholder.
 *
 * Realtime render frequency: unlike 5.8, the 5.7 content-browser pool keeps no in-session cache for
 * on-demand renderers, so OnPropertyChange falls through to an unpopulated package thumbnail and
 * blanks every mosaic on the next refresh (e.g. any OnObjectPropertyChanged from a rebuild). Realtime
 * re-renders loaded collections on each evaluation instead -- cheap, since cells are cached -- so they
 * never blank; the pool only re-renders on real refresh events, not per-frame. Unloaded collections
 * can't render live and fall back to the class icon (previewing those would need a to-disk bake on save).
 */
UCLASS()
