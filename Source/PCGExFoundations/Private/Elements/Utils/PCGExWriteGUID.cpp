...
bool FPCGExGUIDDetails::Init(FPCGExContext* InContext, TSharedRef<PCGExData::FFacade>& InFacade)
{
	switch (Format)
	{
	default: case EPCGExGUIDFormat::Digits:
...
namespace PCGExWriteGUID
{
	bool FProcessor::Process(const TSharedPtr<PCGExMT::FTaskManager>& InTaskManager)
	{
		...
		if (!Config.Init(Context, PointDataFacade))
		{
			return false;
		}
	...
	void FProcessor::ProcessPoints(const PCGExMT::FScope& Scope)
	{
		...
		PCGEX_SCOPE_LOOP(Index)
		{
			...
			{
				IntegerGUIDWriter->SetValue(Index, GetTypeHash(GUID.ToString(Config.GUIDFormat)));
			}
			else
			{
				StringGUIDWriter->SetValue(Index, GUID.ToString(Config.GUIDFormat));
			}
		}
	}
