...
namespace PCGExCopyAttributes
{
		...
		const bool bMaterialize)
	{
		...
		// 5.7 has no FPCGMetadataAttributeDesc: recreate the target as a typed attribute of the source's
		// type, seeded with the source's default value + interpolation and overriding any inherited parent.
		// PropertyCopyAttribute below writes the actual per-slot values.
		FPCGMetadataAttributeBase* TargetAttribute = nullptr;
		PCGExMetaHelpers::ExecuteWithRightType(
			SourceAttribute->GetTypeId(),
			[&](auto DummyValue)
			{
				using T = decltype(DummyValue);
				const FPCGMetadataAttribute<T>* TypedSource = static_cast<const FPCGMetadataAttribute<T>*>(SourceAttribute);
				TargetAttribute = TargetDomain->CreateAttribute<T>(
					TargetName, TypedSource->GetValue(PCGDefaultValueKey),
					SourceAttribute->AllowsInterpolation(), /*bOverrideParent=*/true);
			});
		if (!TargetAttribute)
		{
