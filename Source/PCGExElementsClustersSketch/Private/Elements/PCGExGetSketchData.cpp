...
bool FPCGExGetSketchDataElement::Boot(FPCGExContext* InContext) const
{
	...
	{
		const UPCGComponent* SourceComponent = Context->GetComponent();
		if (!SourceComponent)
		{
		...
		const AActor* Self = SourceComponent->GetOwner();

		TFunction<bool(const AActor*)> BoundsCheck = [](const AActor*) -> bool { return true; };
		if (Settings->ActorSelector.bMustOverlapSelf)
		{
			const FBox SelfBounds = Self ? PCGHelpers::GetGridBounds(Self, SourceComponent) : FBox(ForceInit);
			BoundsCheck = [SelfBounds, SourceComponent](const AActor* Other) -> bool
			{
				const FBox OtherBounds = Other ? PCGHelpers::GetGridBounds(Other, SourceComponent) : FBox(ForceInit);
		...
		Actors = PCGActorSelector::FindActors(Settings->ActorSelector, SourceComponent, BoundsCheck, SelfIgnoreCheck);
	}
