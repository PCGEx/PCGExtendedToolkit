...
namespace PCGExActorBounds
{
	...
	struct FCull
	{
		...
		const FBox* Get() const { return Box.IsValid ? &Box : nullptr; }
	};
	...
	/** Dynamic keys when the selection is overridden or a Bounds input exists. A self-only cull marks the key culled. */
